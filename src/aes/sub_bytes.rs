// TODO: Replace this impl with a none cached one - timing attacks not possible then.
const LOOKUP_TABLE: [u8; 256] = [
    
];

const INV_LOOKUP_TABLE: [u8; 256] = [
    
];

pub fn sub_bytes(state: &mut [u8]) {
    for i in 0..state.len() {
        state[i] = LOOKUP_TABLE[state[i] as usize]
    }
}

pub fn inv_sub_bytes(state: &mut [u8]) {
    for i in 0..state.len() {
        state[i] = INV_LOOKUP_TABLE[state[i] as usize]
    }
}

mod test {
    use super::sub_bytes;
    use super::inv_sub_bytes;
    use string_util::StringUtil;
    use byte_util;

    #[test]
    fn test_sub_bytes() {
        let input = "00102030405060708090a0b0c0d0e0f0".hex_to_bytes();
        let mut actual_array = byte_util::from_slice(input.as_slice());
        sub_bytes(&mut actual_array);

        let expected = "63cab7040953d051cd60e0e7ba70e18c".hex_to_bytes();
        let expected_array = byte_util::from_slice(expected.as_slice());

        assert_eq!(expected_array, actual_array);
    }

    #[test]
    fn test_inv_sub_bytes() {
        let input = "7a9f102789d5f50b2beffd9f3dca4ea7".hex_to_bytes();
        let mut actual_array = byte_util::from_slice(input.as_slice());
        inv_sub_bytes(&mut actual_array);

        let expected = "bd6e7c3df2b5779e0b61216e8b10b689".hex_to_bytes();
        let expected_array = byte_util::from_slice(expected.as_slice());

        assert_eq!(expected_array, actual_array);
    }
}